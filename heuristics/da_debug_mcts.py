...
class TimetableNode:
    ...
    def best_child(self, exploration_weight=0.02):      # Selection of best child node using UCB1 formula for minimization
...
def print_node_path(traversed_nodes, score, soft_score, iteration):
    """Print the path of nodes with their details"""
    print(f"\n--- Iteration {iteration} ---")
    print(f"Score: Hard={score}, Soft={soft_score}")
    print(f"Path length: {len(traversed_nodes)} nodes")
    
    # Print path from leaf to root
    for i, node in enumerate(traversed_nodes):
        depth = len(traversed_nodes) - i - 1  # Calculate depth (root = 0)
        indent = "  " * depth
        
        if node.action:
            exam, period, room_info = node.action
            if isinstance(room_info, list):
                room_str = f"rooms {[r.number for r in room_info]}"
            else:
                room_str = f"room {room_info.number}"
            action_str = f"Exam {exam.number} -> Period {period.number}, {room_str}"
        else:
            action_str = "ROOT"
        
        assigned_count = len(node.state.assigned_exams)
        unassigned_count = len(node.state.unassigned_exams)
        
        print(f"{indent}Node {i}: {action_str} (Assigned: {assigned_count}, Unassigned: {unassigned_count}, Visits: {node.visits})")
...
def mcts_search(problem, time_budget=7200):
    ...
    try:
        ...
        while time.time() < end_time:
            ...
            # 4. Backpropagation
            nodes_list = backpropagate(node, score, soft_score)
            nodes_list.reverse()
            print_node_path(nodes_list, score, soft_score, iteration)
